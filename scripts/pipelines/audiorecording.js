        ...
        for (var n = 0; n < obj.flows[fl].nodes.length; n++) {
            for (var ac = 0; ac < obj.flows[fl].nodes[n].actions.length; ac++) {
                ...
                if (curr_act.type == "send_msg") {
                    ...
                    new_bit.original_text = curr_act.text.trim();

                    // handle contact fields or results
                    new_bit.text_to_read = new_bit.original_text.replace("@results.count ","").replace("@fields.survey_behave_name","your child").replace("@results.n_skills_week","the following").replace("@results.n_skills", "the number of skills in your toolkit");
